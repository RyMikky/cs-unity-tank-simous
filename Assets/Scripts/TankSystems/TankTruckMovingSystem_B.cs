...
public class TankTruckMovingSystem_B : MonoBehaviour
{
    ...
    [Tooltip("Флаг включения дифференциала. Колеса будут крутиться с одной скоростью и момент " +
        "будет скалироваться в зависимости от количества колес, имеющих контакт с поверхностью")]
    public bool _useDufferencial = false;                              // включение дифференциала
    ...
    private bool _needRotation = false;               // флаг необходимости изменить вращение колес
    ...
    [Tooltip("Усредненный угол поворота колес")]
    ...
    void UpdateCurrentSpeed()
    {
        ...
        if (_angularSpeed >= 0.1f || _currentSpeed >= 0.1f)             // при превышении скорости определенного порога
        {
            // запрашиваем поворот мешей колес
            _needRotation = true;
        }
        else
        {
            // отменяем поворот мешей колес
            _needRotation = false;
        }
    }
    ...
    // непосредственное изменения положение коллайдеров колес, вызываемое из функции CompletteDrive()
    ...
    // обновление трансформов колес, костей гусенниц и референсных колес
    ...
    void UpdateWheelTransform(ref Quaternion rotation, ref WheelCollider[] coliders, ref Transform[] transforms, ref Transform[] bones)
    {
        ...
        for (int i = 0; i < transforms.Length; i++)
        {
            ...
            // оброаботка общего угла поворота для мешей всех колес
            {
                if (isFirst)
                {
                    // для первого колеса в массиве записываем дельту угла поворота
                    delta_angle = Quaternion.Angle(transforms[i].rotation, col_rotation);
                    // задаём реверенсному повороту значение текущего коллайдера
                    rotation = col_rotation;

                    if (delta_angle != 0.0f)
                    {
                        // закрываем флаг только в том случае, если угол не рапвен нулю
                        isFirst = false;
                        // может так случиться, что колесо висит в воздухе
                        // соответственно на него не приходит мощность и нет поворота
                    }
                }

                else
                {
                    // получаем текущий угол поворота коллайдера колеса
                    float angle = Quaternion.Angle(transforms[i].rotation, col_rotation);
                    // угол поворота коллайдера данного колеса меньше текущей дельты, то перезаписываем дельту и референс поворота
    ...
    // ----------------------------------------------- неиспользуемые старые версии функций -----------------------------------------
